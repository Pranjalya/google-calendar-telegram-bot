...
def getmyid(update, context):
    ...
    """
    context.bot.send_message(chat_id=update.effective_chat.id, text="The group or chat id is {}".format(update.effective_chat.id))
...
def getevents(update, context):
    ...
    if update.effective_chat.id in AUTH_LIST:
        ...
        answer = "Hey {}, here are your upcoming events in the next 30 days : ".format(update.message.from_user.first_name)
        ...
        for event in events:
            ...
            if event['start'].get('dateTime') is not None:
                answer += "\nDate : " + datetime.fromisoformat(event['start']['dateTime']).strftime("%d-%m-%Y, %A")
                answer += "\nTime : " + datetime.fromisoformat(event['start']['dateTime']).strftime("%I:%M %p GMT %Z")
            elif event['start'].get('date') is not None:
                answer += "\nDate : " + datetime.fromisoformat(event['start']['date']).strftime("%d-%m-%Y, %A")
                answer += "\nTime : Not mentioned"
    ...
    try:
        if jobs.get(context.job.context, None) is not None:
            if context.job.context in AUTH_LIST:
                ...
                context.bot.send_message(chat_id=context.job.context, text="Here are your upcoming events in the next 30 days : ")
                ...
                context.bot.send_message(chat_id=context.job.context, text="Sorry, you are not authorized.")
...
def start_job(update, context):
    ...
    if jobs.get(update.effective_chat.id, None)==None:
        ...
        t = (datetime.now()-timedelta(hours=5, minutes=30)+timedelta(seconds=30)).time()
        ...
        ## fixed_time = time(0,0) 
        print(t, localtime)
        context.bot.send_message(chat_id=update.effective_chat.id, text="I'm going to start sending notifications at {} daily.".format(localtime.strftime("%I:%M %p"))) 
        job_daily = context.job_queue.run_daily(startevents, time=t, days=(0,1,2,3,4,5,6), context=update.effective_chat.id, name="Daily notification at {}".format(t))
        jobs[update.effective_chat.id]=job_daily
    
    else:
        context.bot.send_message(chat_id=update.effective_chat.id, text="You have already scheduled a notifcation sprint at {}".format(jobs.get(update.effective_chat.id).next_t.strftime("%I:%M %p UTC")))


def stop_job(update, context):
    if jobs.get(update.effective_chat.id):
        context.bot.send_message(chat_id=update.effective_chat.id, text="Sure, I'll stop sending notifications.")
        jobs[update.effective_chat.id] = None
    else:
        context.bot.send_message(chat_id=update.effective_chat.id, text="You have no schduled notifications.")
...
def main():
    ...
    updater.dispatcher.add_handler(CommandHandler('notify', start_job, pass_job_queue=True))
    updater.dispatcher.add_handler(CommandHandler('stop', stop_job, pass_job_queue=True))
